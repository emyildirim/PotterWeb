export const translations = {
  "navbar": {
    "home": {
      "en": "Home",
      "es": "Inicio",
      "fr": "Accueil",
      "it": "Home",
      "pt": "Início"
    },
    "all": {
      "en": "All",
      "es": "Todo",
      "fr": "Tout",
      "it": "Tutto",
      "pt": "Tudo"
    },
    "books": {
      "en": "Books",
      "es": "Libros",
      "fr": "Livres",
      "it": "Libri",
      "pt": "Livros"
    },
    "characters": {
      "en": "Characters",
      "es": "Personajes",
      "fr": "Personnages",
      "it": "Personaggi",
      "pt": "Personagens"
    },
    "houses": {
      "en": "Houses",
      "es": "Casas",
      "fr": "Maisons",
      "it": "Case",
      "pt": "Casas"
    },
    "spells": {
      "en": "Spells",
      "es": "Hechizos",
      "fr": "Sorts",
      "it": "Incantesimi",
      "pt": "Feitiços"
    },
    "random": {
      "en": "Random",
      "es": "Aleatorio",
      "fr": "Aléatoire",
      "it": "Casuale",
      "pt": "Aleatório",
    "generate": {
        "en": "Generate",
        "es": "Generar",
        "fr": "Générer",
        "it": "Genera",
        "pt": "Gerar"
      }
    },
    "about": {
      "en": "About",
      "es": "Acerca de",
      "fr": "À propos",
      "it": "Informazioni",
      "pt": "Sobre"
    },
    "language": {
      "en": "Language",
      "es": "Idioma",
      "fr": "Langue",
      "it": "Lingua",
      "pt": "Idioma"
    },
    "languageNames": {
      "en": "English",
      "es": "Español",
      "fr": "Français",
      "it": "Italiano",
      "pt": "Português"
    }
  },
  "homePage": {
    "title": {
      "en": "Welcome to PotterWeb",
      "es": "Bienvenido a PotterWeb",
      "fr": "Bienvenue sur PotterWeb",
      "it": "Benvenuto su PotterWeb",
      "pt": "Bem-vindo ao PotterWeb"
    },
    "description": {
      "en": "PotterWeb is a website that provides information about the Harry Potter series.",
      "es": "PotterWeb es un sitio web que proporciona información sobre la serie de Harry Potter.",
      "fr": "PotterWeb est un site web qui fournit des informations sur la série Harry Potter.",
      "it": "PotterWeb è un sito web che fornisce informazioni sulla serie di Harry Potter.",
      "pt": "PotterWeb é um site web que fornece informações sobre a série de Harry Potter."
    }
  },
  "errors": {
    "noData": {
      "en": "No Data Found!",
      "es": "¡No se encontraron datos!",
      "fr": "Aucune donnée trouvée!",
      "it": "Nessun dato trovato!",
      "pt": "Nenhum dado encontrado!"
    },
    "somethingWentWrong": {
      "en": "Something went wrong, Please try again later.",
      "es": "Algo salió mal, por favor intente de nuevo más tarde.",
      "fr": "Quelque chose a mal tourné, veuillez réessayer plus tard.",
      "it": "Qualcosa è andato storto, per favore riprova più tardi.",
      "pt": "Algo deu errado, por favor tente novamente mais tarde."
    },
    "loading": {
      "en": "Loading...",
      "es": "Cargando...",
      "fr": "Chargement...",
      "it": "Caricamento...",
      "pt": "Carregando..."
    }
  },
  "aboutPage": {
    "title": {
      "en": "About Us",
      "es": "Acerca de Nosotros",
      "fr": "À Propos de Nous",
      "it": "Chi Siamo",
      "pt": "Sobre Nós"
    },
    "ourStory": {
      "en": "Our Story",
      "es": "Nuestra Historia",
      "fr": "Notre Histoire",
      "it": "La Nostra Storia",
      "pt": "Nossa História"
    },
    "ourMission": {
      "en": "Our Mission",
      "es": "Nuestra Misión",
      "fr": "Notre Mission",
      "it": "La Nostra Missione",
      "pt": "Nossa Missão"
    },
    "ourTeam": {
      "en": "Our Team",
      "es": "Nuestro Equipo",
      "fr": "Notre Équipe",
      "it": "Il Nostro Team",
      "pt": "Nossa Equipe"
    },
    "getInTouch": {
      "en": "Get In Touch",
      "es": "Ponte en Contacto",
      "fr": "Contactez-Nous",
      "it": "Contattaci",
      "pt": "Entre em Contato"
    }
  },
  "search": {
    "noResults": {
      "en": "No results found for \"{query}\".",
      "es": "No se encontraron resultados para \"{query}\".",
      "fr": "Aucun résultat trouvé pour \"{query}\".",
      "it": "Nessun risultato trovato per \"{query}\".",
      "pt": "Nenhum resultado encontrado para \"{query}\"."
    },
    "loadingResults": {
      "en": "Loading results...",
      "es": "Cargando resultados...",
      "fr": "Chargement des résultats...",
      "it": "Caricamento dei risultati...",
      "pt": "Carregando resultados..."
    },
    "errorFetching": {
      "en": "There was an error fetching search results.",
      "es": "Hubo un error al obtener los resultados de búsqueda.",
      "fr": "Une erreur s'est produite lors de la récupération des résultats de recherche.",
      "it": "Si è verificato un errore durante il recupero dei risultati della ricerca.",
      "pt": "Ocorreu um erro ao buscar os resultados da pesquisa."
    },
    "placeholder": {
      "en": "Search",
      "es": "Buscar",
      "fr": "Rechercher",
      "it": "Cerca",
      "pt": "Pesquisar"
    },
    "button": {
      "en": "Search",
      "es": "Buscar",
      "fr": "Rechercher",
      "it": "Cerca",
      "pt": "Pesquisar"
    }
  },
  "item": {
    "noData": {
      "en": "No Data Found!",
      "es": "¡Datos no encontrados!",
      "fr": "Aucune donnée trouvée!",
      "it": "Nessun dato trovato!",
      "pt": "Dados não encontrados!"
    },
    "book": {
      "number": {
        "en": "Book",
        "es": "Libro",
        "fr": "Livre",
        "it": "Libro",
        "pt": "Livro"
      },
      "publishDate": {
        "en": "Publish Date",
        "es": "Fecha de publicación",
        "fr": "Date de publication",
        "it": "Data di pubblicazione",
        "pt": "Data de publicação"
      },
      "totalPages": {
        "en": "Total Pages",
        "es": "Páginas totales",
        "fr": "Nombre de pages",
        "it": "Pagine totali",
        "pt": "Total de páginas"
      }
    },
    "character": {
      "birthdate": {
        "en": "Birthdate",
        "es": "Fecha de nacimiento",
        "fr": "Date de naissance",
        "it": "Data di nascita",
        "pt": "Data de nascimento"
      },
      "actor": {
        "en": "Actor",
        "es": "Actor",
        "fr": "Acteur",
        "it": "Attore",
        "pt": "Ator"
      },
      "children": {
        "en": "Children",
        "es": "Hijos",
        "fr": "Enfants",
        "it": "Figli",
        "pt": "Filhos"
      },
      "na": {
        "en": "N/A",
        "es": "N/D",
        "fr": "N/D",
        "it": "N/D",
        "pt": "N/D"
      }
    },
    "house": {
      "founder": {
        "en": "Founder",
        "es": "Fundador",
        "fr": "Fondateur",
        "it": "Fondatore",
        "pt": "Fundador"
      },
      "colors": {
        "en": "Colors",
        "es": "Colores",
        "fr": "Couleurs",
        "it": "Colori",
        "pt": "Cores"
      },
      "and": {
        "en": "and",
        "es": "y",
        "fr": "et",
        "it": "e",
        "pt": "e"
      },
      "animal": {
        "en": "Animal",
        "es": "Animal",
        "fr": "Animal",
        "it": "Animale",
        "pt": "Animal"
      },
      "red": {
        "en": "Red",
        "es": "Rojo",
        "fr": "Rouge",
        "it": "Rosso",
        "pt": "Vermelho"
      },
      "green": {
        "en": "Green",
        "es": "Verde",
        "fr": "Vert",
        "it": "Verde",
        "pt": "Verde"
      },
      "blue": {
        "en": "Blue",
        "es": "Azul",
        "fr": "Bleu",
        "it": "Blu",
      },
      "yellow": {
        "en": "Yellow",
        "es": "Amarillo",
        "fr": "Jaune",
        "it": "Giallo",
        "pt": "Amarelo"
      },
      "black": {
        "en": "Black",
        "es": "Negro",
        "fr": "Noir",
        "it": "Nero",
        "pt": "Preto"
      },
      "gold": {
        "en": "Gold",
        "es": "Oro",
        "fr": "Or",
        "it": "Oro",
        "pt": "Ouro"
      },
      "silver": {
        "en": "Silver",
        "es": "Plata",
        "fr": "Argent",
        "it": "Argento",
        "pt": "Prata"
      }
    },
    "spell": {
      "use": {
        "en": "Use",
        "es": "Uso",
        "fr": "Utilisation",
        "it": "Utilizzo",
        "pt": "Uso"
      }
    },
    "error": {
      "en": "Something went wrong, Please try again later.",
      "es": "Algo salió mal, por favor intente más tarde.",
      "fr": "Une erreur s'est produite, veuillez réessayer plus tard.",
      "it": "Qualcosa è andato storto, riprova più tardi.",
      "pt": "Algo deu errado, tente novamente mais tarde."
    }
  }
}
